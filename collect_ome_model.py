...
def fetch_openmeteo_model_forecast(station: dict, params: Dict[str, Any] | None = None) -> List[dict]:
    """
    Model-specific Open-Meteo endpoints (hourly) -> daily highs/lows (today+tomorrow)
    params:
      {"model": "gfs" | "ecmwf" | "gem" | "icon"}
    """
    lat = station.get("lat")
    lon = station.get("lon")
